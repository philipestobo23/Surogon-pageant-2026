...
class FinalsController extends Controller
{
    public function final_form(){
        ...
        $candidates = Finals::select('*', $currentjudge . '_final')->take(5)->get();
        $data = [];
    ...
    public function rank_final(){
        ...
        $rankContestants = Finals::select(['*', \DB::raw('RANK() OVER (ORDER BY '. $score .' DESC) AS row_id')])->take(5)->get();
        foreach ($rankContestants as $item) {
            // Update the rank column
            Finals::where('id', $item->id)->update([$rank => $item->row_id]); 
        }

        $sortedRanking = Finals::orderBy($rank, 'asc')->take(5)->get();
        $data = [];
