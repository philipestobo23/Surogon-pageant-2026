...
<div class="judge-dashboard">
    ...
    <div class="welcome-banner">
        <div class="wb-inner">
            ...
            <div class="wb-text">
                ...
                <span class="wb-badge"><i class="bi bi-shield-check me-1"></i>Authorized Panel — Surogon Pageant 2026</span>
            </div>
            <div class="wb-crown">
    ...
    <div class="dashboard-grid">
        ...
        <div class="section-card card-violet">
            <div class="sc-header">
                ...
                <div>
                    ...
                    <h3 class="sc-title">Top 10 Selection</h3>
                </div>
                <span class="sc-badge">3 Events</span>
            </div>

            <p class="sc-desc">Score all contestants for the Top 10 qualifying round. Each event is judged independently.</p>
            ...
            <div class="sc-buttons">
                ...
                <a href="{{ route('production_wear_form') }}" class="judge-btn btn-filipiniana">
                    <span class="jb-icon"><i class="bi bi-flag-fill"></i></span>
                    <span class="jb-body">
                        <span class="jb-title">Filipiniana Wear</span>
                        <span class="jb-sub">Cultural costume scoring</span>
                    </span>
                    <i class="bi bi-chevron-right jb-arrow"></i>
                </a>
            </div>
        ...
        <div class="section-card card-teal">
            ...
            <p class="sc-desc">Score the shortlisted Top 10 contestants for the Snap Talk question round to determine the Top 5.</p>
        ...
        <div class="section-card card-rose">
            ...
            <p class="sc-desc">Determine the ultimate winner. Score the Top 5 finalists in the final Q&amp;A to crown Binibining Surigay 2026.</p>
    ...
    <div class="cosmos-marquee">
        <div class="cm-track">
            @for ($i = 0; $i < 3; $i++)
                <span><i class="bi bi-star-fill"></i> Grand Coronation Night of the Binibining Surigay 2025</span>
                ...
                <span><i class="bi bi-star-fill"></i> Surogon Pageant 2026</span>
            @endfor
        </div>
    </div>
    ...
    <footer class="dash-footer">
        ...
        <p>Developed by <span>Provincial ICT Office</span></p>
...
.card-violet::after  { background: linear-gradient(90deg, #aa44ff, #6600cc); }
.card-teal::after    { background: linear-gradient(90deg, #00ccaa, #0066aa); }
.card-rose::after    { background: linear-gradient(90deg, #ff4488, #aa0066); }
...
.violet-glow { background: rgba(110,0,220,0.35); color: #cc77ff; box-shadow: 0 0 16px rgba(160,60,255,0.35); }
.teal-glow   { background: rgba(0,140,120,0.35);  color: #33ddbb; box-shadow: 0 0 16px rgba(0,200,160,0.30); }
.rose-glow   { background: rgba(180,0,80,0.35);   color: #ff77aa; box-shadow: 0 0 16px rgba(255,40,110,0.35); }
...
.sc-badge-final {
    background: rgba(180,0,80,0.30);
    border-color: rgba(255,60,130,0.35);
    color: #ff88bb;
}
...
.judge-btn:hover::before { opacity: 1; }
.judge-btn:hover { transform: translateX(3px); text-decoration: none; }
.judge-btn:active { transform: translateX(0) scale(0.98); }

/* swimwear */
.btn-swimwear           { background: rgba(0, 170, 200, 0.18); border-color: rgba(0,200,230,0.35); }
.btn-swimwear::before   { background: rgba(0,200,230,0.08); }
.btn-swimwear .jb-icon  { color: #33ddee; background: rgba(0,180,210,0.20); }
.btn-swimwear:hover      { border-color: rgba(0,220,255,0.6); box-shadow: 0 0 18px rgba(0,200,230,0.25); }

/* gown */
.btn-gown               { background: rgba(180, 100, 200, 0.18); border-color: rgba(200,100,220,0.30); }
.btn-gown::before       { background: rgba(200,100,220,0.08); }
.btn-gown .jb-icon      { color: #dd88ff; background: rgba(180,80,200,0.22); }
.btn-gown:hover          { border-color: rgba(220,110,255,0.6); box-shadow: 0 0 18px rgba(180,80,220,0.25); }

/* filipiniana */
.btn-filipiniana           { background: rgba(90, 110, 200, 0.18); border-color: rgba(100,120,220,0.30); }
.btn-filipiniana::before   { background: rgba(100,120,220,0.08); }
.btn-filipiniana .jb-icon  { color: #99aaff; background: rgba(80,100,200,0.22); }
.btn-filipiniana:hover      { border-color: rgba(120,140,255,0.6); box-shadow: 0 0 18px rgba(100,120,220,0.25); }

/* snaptalk */
.btn-snaptalk           { background: rgba(0, 170, 130, 0.18); border-color: rgba(0,200,160,0.30); }
.btn-snaptalk::before   { background: rgba(0,200,160,0.08); }
.btn-snaptalk .jb-icon  { color: #33ddbb; background: rgba(0,170,130,0.22); }
.btn-snaptalk:hover      { border-color: rgba(0,220,180,0.6); box-shadow: 0 0 18px rgba(0,200,160,0.25); }

/* final */
.btn-final              { background: rgba(200, 20, 80, 0.20); border-color: rgba(220,40,100,0.35); }
.btn-final::before      { background: rgba(220,40,100,0.08); }
.btn-final .jb-icon     { color: #ff77aa; background: rgba(200,20,80,0.25); }
.btn-final:hover         { border-color: rgba(255,60,130,0.6); box-shadow: 0 0 20px rgba(220,20,80,0.30); }
...
@media (min-width: 768px) and (max-width: 1023px) {
    .judge-dashboard { padding: 22px 18px; }
    .welcome-banner   { padding: 20px 22px; }
    .wb-crown         { display: none; }
    .section-card:last-child { grid-column: 1 / -1; }
}

/* ─── small mobile ────────────────────────────────────────── */
@media (max-width: 500px) {
    .wb-avatar { font-size: 2.4rem; }
    .wb-name   { font-size: 1.1rem; }
    .wb-badge  { font-size: 0.62rem; }
    .wb-crown  { display: none; }
...
.hd-orb-1 { width: 520px; height: 520px; background: radial-gradient(circle, rgba(120,0,255,0.38) 0%, transparent 70%);  top: -10%;  left: -8%;   --dur:14s; --delay:0s;   }
.hd-orb-2 { width: 420px; height: 420px; background: radial-gradient(circle, rgba(60,0,180,0.32) 0%,  transparent 70%);  top:  20%;  right: -6%;  --dur:11s; --delay:2s;   }
.hd-orb-3 { width: 600px; height: 350px; background: radial-gradient(ellipse, rgba(80,20,200,0.28) 0%, transparent 70%); bottom:-5%; left:  15%;  --dur:16s; --delay:4s;   }
.hd-orb-4 { width: 300px; height: 300px; background: radial-gradient(circle, rgba(160,40,255,0.25) 0%, transparent 70%); top:  45%;  left:  40%;  --dur:9s;  --delay:1.5s; }
.hd-orb-5 { width: 380px; height: 260px; background: radial-gradient(ellipse, rgba(30,0,120,0.30) 0%,  transparent 70%); top:  5%;   right: 25%;  --dur:13s; --delay:3s;   }

@keyframes orbPulse {
    0%,100% { opacity: 0;    transform: scale(0.92); }
    40%,60% { opacity: 1;    transform: scale(1.06); }
}
...
@keyframes edgePulse {
    0%,100% { opacity: 0.5; }
    50%     { opacity: 1; box-shadow: 0 0 20px rgba(180,80,255,0.6); }
}

/* ── make all dashboard content sit above bg ────────────── */
.judge-dashboard { position: relative; z-index: 2; }
